...
export function Procedures() {
  ...
  const { procedures } = getSiteData(language)
  ...
  const selectedProcedure =
    selectedProcedureIndex === null ? null : procedures[selectedProcedureIndex]
  ...
  return (
    <section className="procedures section-shell" id="procedimentos">
      <div className="section-heading">
        <span className="eyebrow">{copy.procedures.eyebrow}</span>
        <h2>{copy.procedures.title}</h2>
        <p>{copy.procedures.intro}</p>
      </div>
      ...
      <div className="procedure-carousel">
        {currentProcedure > 0 ? (
          <button
            ...
            type="button"
            aria-label={copy.procedures.previous}
        ...
        <div className="procedure-grid" ref={carouselRef}>
          {procedures.map((procedure, index) => (
            <Card
              ...
              ariaLabel={`${copy.procedures.open} ${procedure.title}`}
        ...
        {currentProcedure < procedures.length - 1 ? (
          <button
            ...
            type="button"
            aria-label={copy.procedures.next}
      ...
      <AnimatePresence>
        {selectedProcedure ? (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={() => setSelectedProcedureIndex(null)}
          >
            <motion.div
            ...
            >
              <button
                ...
                type="button"
                aria-label={copy.procedures.close}
                onClick={() => setSelectedProcedureIndex(null)}
              >
              ...
              <span className="eyebrow">{copy.procedures.modalEyebrow}</span>
